...
const cardBase =
  "rounded-3xl border border-sky-200/70 bg-white/90 shadow-[0_10px_28px_rgba(20,42,82,0.10)]";

const chipBase =
  "rounded-full border border-sky-200 bg-sky-50/80 px-3 py-1.5 text-xs font-medium text-slate-700";
...
export function FramerInspiredPortfolio() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#f7fbff] via-[#fffaf3] to-[#f6f8ff] text-slate-900 selection:bg-sky-200/70">
      <div className="pointer-events-none fixed inset-0 opacity-60">
        ...
        <div className="absolute -left-20 top-0 h-80 w-80 rounded-full bg-sky-200/30 blur-3xl" />
        <div className="absolute -right-20 bottom-20 h-80 w-80 rounded-full bg-amber-200/25 blur-3xl" />
      </div>

      <header className="sticky top-0 z-50 border-b border-sky-200/70 bg-white/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 md:px-6">
          ...
          <nav className="hidden items-center gap-2 md:flex">
            <a href="#work" className={`${chipBase} hover:bg-sky-100`}>Works</a>
            <a href="#experience" className={`${chipBase} hover:bg-sky-100`}>Experience</a>
            <a href="#contact" className={`${chipBase} hover:bg-sky-100`}>Contact</a>
          </nav>
          ...
          <div className="flex items-center gap-2 text-sm">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className={`${chipBase} hover:bg-sky-100`}
            >
              LinkedIn
            </a>
            <a
      ...
      <main id="top" className="relative z-10 mx-auto max-w-6xl px-4 pb-24 pt-8 md:px-6 md:pt-10">
        <section className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <div className={`${cardBase} p-6 md:p-8 lg:p-10`}>
            <div className="flex flex-wrap items-center gap-3">
              <span className="rounded-full bg-emerald-100/90 px-3 py-1 text-xs font-semibold text-emerald-700">
                Available for work
              </span>
              <span className="rounded-full border border-amber-200 bg-amber-50/80 px-3 py-1 text-xs font-medium text-amber-700">
            ...
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <a
                href="#work"
                className="rounded-full bg-gradient-to-r from-sky-600 to-indigo-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:from-sky-500 hover:to-indigo-500"
              >
                View Work
              </a>
              <a
                href={`mailto:${PERSONAL_INFO.email}`}
                className="rounded-full border border-amber-300 bg-amber-50/80 px-5 py-2.5 text-sm font-semibold text-amber-800 transition hover:-translate-y-0.5 hover:border-amber-400"
              >
                Start a Project
              </a>
            </div>
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-3">
              {stats.map((item) => (
                <div key={item.label} className="rounded-2xl border border-sky-200/70 bg-gradient-to-br from-sky-50 to-white p-4">
          ...
          <div className="grid gap-4">
            <div className={`${cardBase} overflow-hidden p-3`}>
              ...
              <img
                ...
                className="h-[340px] w-full rounded-2xl border border-sky-200/70 object-cover md:h-[420px]"
              />
            </div>

            <div className={`${cardBase} bg-gradient-to-br from-indigo-50/80 to-sky-50/80 p-5`}>
        ...
        <section className="mt-6 grid gap-3 md:grid-cols-[1fr_auto]">
          <div className={`${cardBase} flex flex-wrap items-center gap-2 bg-gradient-to-r from-white to-sky-50/70 p-4`}>
          ...
          <a
            href={`mailto:${PERSONAL_INFO.email}`}
            className="inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-indigo-600 to-sky-600 px-6 py-4 text-sm font-semibold text-white transition hover:from-indigo-500 hover:to-sky-500"
          >
            {PERSONAL_INFO.email}
          </a>
        </section>
        ...
        <section id="work" className="mt-14">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {workItems.map((project, index) => (
              ...
              >
                <div className="border-b border-sky-200/70 bg-sky-50/40">
                ...
                <div className="p-5 md:p-6">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {project.tech.slice(0, 4).map((tech) => (
                      <span key={tech} className={`${chipBase} border-indigo-200/70 bg-indigo-50/70`}>
                        {tech}
                      </span>
                    ))}
                  </div>
        ...
        <section id="contact" className="mt-14">
          <a href={`mailto:${PERSONAL_INFO.email}`} className="block">
            <div className="relative overflow-hidden rounded-[30px] bg-gradient-to-r from-indigo-700 via-sky-700 to-cyan-700 px-6 py-14 text-white md:px-10 md:py-16">
              <div className="absolute -right-16 -top-16 h-56 w-56 rounded-full bg-white/20 blur-3xl" />
              <div className="absolute -left-12 bottom-0 h-44 w-44 rounded-full bg-amber-200/30 blur-3xl" />
              <p className="text-sm uppercase tracking-[0.18em] text-slate-100/90">Get in touch</p>
              <h2 className="mt-2 text-4xl font-semibold leading-tight tracking-tight md:text-6xl">
                Let&apos;s build your next AI product.
              </h2>
              <p className="mt-6 text-lg text-slate-100">{PERSONAL_INFO.email}</p>
            </div>
          </a>
        </section>
      </main>
